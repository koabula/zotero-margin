...
const button = (action: string, label: string, symbol: string) => `<button type="button" class="margin-icon-button" data-action="${action}" aria-label="${label}" title="${label}">${icon(symbol)}</button>`;
...
export class Sidebar {
  ...
  constructor(body: HTMLElement, private host: UIHost) {
    ...
    setMarkup(this.root, `
      <header class="margin-header"><div class="margin-context"><span class="margin-document-title">打开一篇文献</span><span class="margin-document-meta">打开文献开始阅读</span></div><nav aria-label="助手操作">${button("new", "新对话", "plus")}${button("history", "历史对话", "history")}${button("settings", "模型设置", "settings")}</nav></header>
      <main class="margin-chat"><div class="margin-transcript" role="log" aria-label="阅读对话"></div><div class="margin-note-review" hidden="hidden"></div><div class="margin-status" role="status" aria-live="polite"></div><div class="margin-error" role="alert" hidden="hidden"></div>
      <form class="margin-composer"><div class="margin-selection" hidden="hidden"><span></span>${button("selection", "移除选区上下文", "close")}</div><textarea aria-label="向文献提问" rows="2" placeholder="问问这篇文献…"></textarea><div class="margin-composer-bottom"><select class="margin-model" aria-label="聊天模型"></select><button class="margin-send" type="submit" aria-label="发送问题" title="发送 · Enter">${icon("arrow")}</button></div></form></main>
      <section class="margin-settings" hidden="hidden" aria-label="模型设置"></section><section class="margin-history" hidden="hidden" aria-label="历史对话"></section>`);
  ...
  private updateContext(): void {
    ...
    this.el(".margin-document-title").textContent = context?.title || "打开一篇文献";
    this.el(".margin-document-title").title = context?.title || "";
    this.el(".margin-document-meta").textContent = context ? `第 ${context.pageLabel} 页 · PDF ${context.pageIndex + 1}/${context.pageCount}` : "在 Zotero 阅读器里开始";
    const selected = !!context?.selection && this.includeSelection;
    this.show(".margin-selection", selected);
    if (selected) this.el(".margin-selection span").textContent = `已选文字 · ${context!.selection.slice(0, 65)}${context!.selection.length > 65 ? "…" : ""}`;
  ...
  private updateModel(): boolean {
    ...
    if (modelIDs.length && !modelIDs.includes(this.session.modelID || '')) {
      if (this.session.modelID) { removed = true; this.status('原模型已移除，已切换到默认模型'); }
      this.session.modelID = defaultModelID; this.deferSave();
    }
    const options = modelIDs.length ? modelIDs.map(id => `<option value="${escapeHTML(id)}">${escapeHTML(id)}</option>`).join('') : '<option value="">请先配置模型</option>';
    setMarkup(select, options); select.value = this.session.modelID || defaultModelID;
    select.title = select.value || '请在右上角设置模型'; select.disabled = !!this.controller || this.preparing || !modelIDs.length;
    return removed;
  }
  ...
  private renderMessages(): void {
    ...
    if (!this.session.messages.length) {
      ...
      setMarkup(transcript, `<section class="margin-welcome"><span class="margin-welcome-icon">${icon("book")}</span><h2>一起读懂这篇文献</h2><p>选中一段文字，或直接提问</p><div class="margin-suggestions"><button data-prompt="请解释我选中的这段文字，说明关键概念，并举一个简单例子。">解释选区</button><button data-prompt="请总结当前页的核心观点，区分结论与论证，并引用原文。">总结本页</button></div></section>`);
      return;
    }
    ...
    for (const message of this.session.messages) {
      ...
      if (!entry) {
        ...
        entry = {node:holder.firstElementChild as HTMLElement, markup};
        this.messageNodes.set(message.id, entry); transcript.append(entry.node);
      } else if (entry.markup !== markup && !this.selection.blocks(entry.node)) {
        const holder = this.root.ownerDocument.createElementNS('http://www.w3.org/1999/xhtml', 'div');
        setMarkup(holder, markup);
        entry.node.replaceChildren(...Array.from(holder.firstElementChild!.childNodes)); entry.markup = markup;
      }
    }
  ...
  private busy(value: boolean): void {
    ...
    send.title = value ? "停止生成" : "发送 · Enter";
    send.setAttribute("aria-label", value ? "停止生成" : "发送问题");
  ...
  async send(question?: string): Promise<void> {
    ...
    if (!text) return;
    if (text.length > 12000) { this.error(new Error("问题过长，请控制在 12000 字以内。")); return; }
    this.clearError();
    this.preparing = true; this.updateModel();
    const bridge = this.bridge, requestSession = this.session;
    try { this.config = normalizeConfig(await this.host.store.getConfig()); this.updateModel(); endpoint(this.config.baseURL); if (!this.config.defaultModelID) throw new Error("请先配置模型。"); }
    ...
    this.session.messages.push({ id: uid(), role: "user", content: text, sources: [], contextLabel: `第 ${context.pageLabel} 页${context.selection ? " · 已选文字" : ""}` }, answer);
    ...
    try { await agent.run(text, history, controller.signal); }
      ...
      if (this.controller === controller) { this.controller = undefined; this.busy(false); this.status(answer.status === "stopped" ? "已停止" : answer.status === "error" ? "请求未完成，可再次提问" : "回答完成"); }
  ...
  private reviewNote(title: string, content: string, sources: Source[], signal: AbortSignal): Promise<{ saved: boolean; id?: number }> {
    return new Promise(resolve => {
      ...
      setMarkup(this.el(".margin-note-review"), `<div class="margin-review-label">${icon("note")} 笔记草稿</div><strong>${escapeHTML(title)}</strong><details><summary>预览内容</summary><div class="margin-prose">${renderMarkdown(content, sources)}</div></details><div class="margin-review-actions"><button type="button" class="margin-secondary" data-action="dismiss-note">暂不保存</button><button type="button" class="margin-primary" data-action="confirm-note">保存到文献</button></div>`);
      this.show(".margin-note-review", true);
    });
  }
  private async saveNote(title: string, content: string, sources: Source[]): Promise<{ id: number }> {
    if (!this.bridge || !this.context) throw new Error("请先打开文献。");
  ...
  private async action(action: string, target: HTMLElement): Promise<void> {
    if (action === 'fetch-models') {
      ...
      target.textContent = '取消获取';
      const feedback = this.el('.margin-model-feedback'); feedback.textContent = '正在获取模型…';
      try {
        ...
        feedback.textContent = ids.length ? `获取到 ${ids.length} 个模型，请勾选需要的模型。` : '服务返回空列表，请手动添加模型 ID。';
      } catch (e) { if (this.discoveryController === controller) feedback.textContent = e instanceof Error ? e.message : String(e); }
      finally { if (this.discoveryController === controller) { this.discoveryController = undefined; target.textContent = '获取模型'; } }
      return;
    }
    if (action === 'add-model') {
      const input = this.el<HTMLInputElement>('[name="manualModel"]'), id = input.value.trim();
      if (!id) { this.el('.margin-model-feedback').textContent = '请输入模型 ID'; return; }
      ...
      this.el('.margin-model-feedback').textContent = '已添加，保存配置后生效'; return;
    }
    ...
    if (action === "copy") { const message = this.message(target); if (message) { this.host.copy(message.content.replace(/\[\[([^\]]+)\]\]/g, (_, id) => { const source = message.sources.find(s => s.id === id); return source ? `[第 ${source.pageLabel} 页]` : "[出处未验证]"; })); this.status("已复制回答"); } return; }
    if (action === "save-answer") { const message = this.message(target); if (message) { (target as HTMLButtonElement).disabled = true; try { await this.saveNote("阅读笔记 · " + this.context?.title.slice(0, 80), message.content, message.sources); this.status("已保存到文献笔记"); } finally { (target as HTMLButtonElement).disabled = false; } } return; }
    ...
    if (action === "new") {
      if (this.controller || this.preparing) { this.error(new Error("请先停止当前回答，再开始新对话。")); return; }
    ...
    if (action === "restore") {
      if (this.controller || this.preparing) throw new Error("请先停止当前回答。");
    ...
    if (action === "save-settings" || action === "test-settings") {
      const config = this.readSettings(); endpoint(config.baseURL); if (!config.modelIDs.length) throw new Error("请至少选择或添加一个模型。");
      const feedback = this.el(".margin-settings-feedback"); feedback.textContent = "";
      (target as HTMLButtonElement).disabled = true;
      try {
        if (action === "save-settings") { await this.host.store.saveConfig(config); this.config = config; const removed = this.updateModel(); this.setView("chat"); this.status(removed ? "模型配置已保存 · 原模型已移除，已切换到默认模型" : "模型配置已保存"); }
        else { this.connectionController?.abort(); this.connectionController = new AbortController(); feedback.textContent = "正在检查连接与工具调用…"; await testConnection(config, this.connectionController.signal); feedback.textContent = "连接成功，工具调用可用。"; }
  ...
  private openSettings(): void {
    ...
    setMarkup(this.el('.margin-settings'), `<div class="margin-section-heading">${button('back', '返回对话', 'back')}<span>模型设置</span></div>
      ...
      <label>API Key<input name="apiKey" type="password" placeholder="本机服务可留空" autocomplete="off" value="${escapeHTML(this.config.apiKey)}"/></label>
      <div class="margin-model-heading"><strong>可用模型</strong><button type="button" class="margin-secondary" data-action="fetch-models">获取模型</button></div>
      <input name="modelSearch" aria-label="搜索模型" placeholder="搜索模型 ID…"/>
      <div class="margin-model-list" aria-label="选择可用模型"></div>
      <div class="margin-manual-model"><input name="manualModel" aria-label="手动模型 ID" placeholder="手动输入模型 ID"/><button type="button" class="margin-secondary" data-action="add-model">添加</button></div>
      <p class="margin-model-feedback" role="status"></p>
      <label>默认模型<select name="defaultModel" aria-label="默认模型"></select></label>
      <p class="margin-settings-intro">新对话使用默认模型。列表中的模型不一定支持工具调用，可测试默认模型的连接。</p>
      <div class="margin-settings-actions"><button type="button" class="margin-secondary" data-action="test-settings">测试连接</button><button type="button" class="margin-primary" data-action="save-settings">保存配置</button></div>
      <p class="margin-settings-feedback" role="status"></p>
      <div class="margin-privacy"><p>使用兼容 OpenAI Chat Completions 的服务。提问时会发送相关原文和对话；获取模型列表不发送文献。</p><p>密钥保存在 Zotero 凭据库。设置只在点击保存后生效。</p></div>`);
    this.el('[name="modelSearch"]').addEventListener('input', () => this.renderModelList());
    for (const name of ['baseURL','apiKey']) this.el(`[name="${name}"]`).addEventListener('input', () => { this.discoveryController?.abort(); this.discoveryController = undefined; this.el('[data-action="fetch-models"]').textContent = '获取模型'; this.el('.margin-model-feedback').textContent = ''; this.availableModels = [...this.settingsIDs]; this.renderModelList(); });
  ...
  private renderModelList(): void {
    ...
    setMarkup(this.el('.margin-model-list'), ids.length ? ids.map(id => `<label class="margin-model-option"><input type="checkbox" data-model-id="${escapeHTML(id)}" ${this.settingsIDs.has(id) ? 'checked="checked"' : ''}/><span title="${escapeHTML(id)}">${escapeHTML(id)}</span></label>`).join('') : `<p>${query ? '没有匹配的模型' : '获取模型列表，或在下方手动添加'}</p>`);
    const select = this.el<HTMLSelectElement>('[name="defaultModel"]');
    setMarkup(select, this.settingsIDs.size ? [...this.settingsIDs].map(id => `<option value="${escapeHTML(id)}">${escapeHTML(id)}</option>`).join('') : '<option value="">请先选择模型</option>');
  ...
  private openHistory(): void {
    ...
    setMarkup(this.el(".margin-history"), `<div class="margin-section-heading">${button("back", "返回对话", "back")}<span>这篇文献的对话</span></div><p class="margin-settings-intro">每次新对话，都会留在这里。</p>${this.session.archives?.length ? this.session.archives.map((a, i) => `<button class="margin-history-item" data-action="restore" data-index="${i}"><strong>${escapeHTML(a.messages.find(m => m.role === "user")?.content.slice(0, 80) || "阅读对话")}</strong><small>${escapeHTML(new Date(a.createdAt).toLocaleString())} · ${a.messages.length} 条消息</small></button>`).join("") : '<div class="margin-history-empty">还没有历史对话</div>'}`);
  }
  focusInput(): void { this.setView("chat"); this.host.focusPane?.(); this.el("textarea").focus(); }
  dispose(): void { this.stop(); this.connectionController?.abort(); this.discoveryController?.abort(); this.selection.dispose(); clearTimeout(this.renderTimer); clearTimeout(this.saveTimer); void this.persist().catch(() => {}); this.disposed = true; this.root.remove(); }
}
