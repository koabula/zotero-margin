...
> 当前版本 **0.3.0**，支持 **Zotero 10.0.x** 中具有文字层的 PDF。原生界面已在 Windows / Zotero 10.0.1 验证；其他操作系统尚未完成原生验收。
...
1. 前往 [Releases](https://github.com/koabula/zotero-margin/releases/latest)，下载 `margin-0.3.0.xpi`。请下载 XPI 安装包，而不是 GitHub 自动生成的源码压缩包。
...
0.3.0 已通过 **37 项自动化测试**，并在独立的 Zotero 10.0.1 配置中验证语言切换、历史列表布局、侧栏布局、原生复制、文献切换、模型请求、历史恢复和插件清理。模型协议测试使用**本机模拟服务**，不代表真实 LLM 的阅读质量，也不表示已验证所有服务商。详见 [验收记录](VALIDATION.md)。
...
`npm run check` 运行类型检查、测试和构建，生成 `dist/margin-0.3.0.xpi`。更多源码结构、浏览器预览与原生集成测试说明见 [开发文档](docs/DEVELOPMENT.zh-CN.md)。
